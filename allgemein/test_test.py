...
def test_02():
    ...
    if voltage.returncode == 0:
        spannung = float(voltage.stdout.replace("volt=","").replace("V\n","").strip())
    ...
    if x.returncode == 0:
        ...
        x = subprocess.run(cmd, shell=True, \
                                 capture_output=True, text=True)
        if x.returncode == 0:
            ergebnis = x.stdout[x.stdout.find(":")+2:].replace("\n","").strip()
            print(f"Rechnertyp    : {ergebnis:s}")
        else:
            print(c.lightred, f"Errorcode {x.returncode:3d} : ", \
                  f"Befehl '{cmd:s}' unbekannt", c.reset, sep="")
    ...
    x = subprocess.run(cmd, shell=True, \
                             capture_output=True, text=True)
    if x.returncode == 0:
        ergebnis = x.stdout.replace("Description:","").replace("\n","").strip()
...
def _main():
    try:
        ...
        print(3*" ", "\n!!! Abbruch mit Ctrl-C (SIGINT)oder CTRL-Z (SIGTSTP) !!!\n", c.reset)
