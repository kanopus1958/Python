...
def sensoren():
    ...
    temp_min = 100.0
    temp_max = 0.0
    print("Messung gestartet: ", aktuelle_uhrzeit(),
          " auf System '", socket.gethostname(), "'", sep="")
    print("   Uhrzeit    T-akt   T-min   T-max  Spannung     Takt")
    while not stop:
        messergebnis = os.popen("vcgencmd measure_temp").readline()
        temperatur = float(
            (messergebnis.replace("temp=", "").replace("'C\n", "")))
        ...
        spannung = float(
            (messergebnis.replace("volt=", "").replace("V\n", "")))
        ...
        ausgabe = "   {0:8s}  "+c.yellow+"{1:4.1f}'C"+c.reset+"  " + \
                  c.lightblue + \
                  "{2:4.1f}'C"+c.reset+"  "+c.red+"{3:4.1f}'C" + \
                  c.reset+"   " + \
                  "{4:6.4f}V  {5:4d}MHz\r"
