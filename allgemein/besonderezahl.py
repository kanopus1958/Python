...
def tasten_thread(ev):
    try:
        logging.info(
            f"Thread gestartet : {sys._getframe().f_code.co_name} \
              {threading.get_ident()}")
        while not ev.is_set():
            char = input()
            ...
            sleep(0.1)
        logging.info(
            f"Thread gestoppt : {sys._getframe().f_code.co_name} \
              {threading.get_ident()}")
        return
...
def anzeige_werte(tab, index, startzeit):
    ...
    print(f"Persistence-Berechnungen gestartet      (Stop = 'q'+<Enter>)",
          sep="")
    print(60*"*", sep="")
    if len(tab) > 0:
        z = 0
...
def _main():
    ...
    proc = Process(target=besonderezahl_proc, args=(
        proc_child_conn, t0, GC_max_zahl,))
    proc.start()
    ...
    try:
        logging.info(
            f"Berechnung gestartet : {sys._getframe().f_code.co_name}")
        ...
        if not ev_t.is_set():
            ...
            print(c.lightred, f"Programm beenden mit 'q' <Enter> ", c.reset,
                  sep="")
        th_tasten.join()
        print(p.up, f"\r     ", sep="")
        anzeige_werte(pers_tab, bearbeitet, t0)
        if not ev_t.is_set():
            print(f"Persistence-Berechnungen fehlerfrei abgeschlossen",
                  sep="")
        else:
            print(c.lightred,
                  f"Persistence-Berechnungen interaktiv abgebrochen",
                  c.reset, sep="")
        print()
        logging.info(
            f"Thread gestoppt : {sys._getframe().f_code.co_name} \
        ...
        print(2*p.up)
        print("\r***\r", sep="", end="")
        print(c.lightred, "\nAbbruch mit CTRL-C um ", aktuelle_uhrzeit(),
              " (Laufzeit = ", zeit_diff(t0), ")", "\n", c.reset, sep='')
        sys.exit(0)
