...
def synchronisation():
    ...
    for z in ziel_rechner:
        if z != dead_ziel:
            cmd = cmd_ping+" "+z
            x = subprocess.run(cmd, shell=True, stdout=fobj1,
                               stderr=subprocess.STDOUT)
            if x.returncode == 0:
                print(
                      f"*  Datei-Transfer startet    "
                      f": {knotenname:11s}--> {z:11s}", " *\r")
                print(
                      f"*  {rsync_param[i][:rsync_param[i].find(' ')]:50s}",
                      4*" ", "*\r")
                cmd = cmd_rsync+" "+rsync_param[i]
                x = subprocess.run(
                    cmd, shell=True, stdout=fobj2, stderr=subprocess.STDOUT)
                if x.returncode == 0:
                    print(f"*  Datei-Transfer erfolgreich", 28*" ", "*\r")
                else:
                    print(
                          "*", c.lightred,
                          f"Datei - Transfer fehlgeschlagen !",
                          c.yellow, 20*" ", "*\r")
            elif x.returncode == 1:
                dead_ziel = z
                print(
                      "*", c.lightcyan, f"Zielrechner ausgeschaltet : {z:12s}",
                      c.yellow, 13*" ", "*\r")
                print("*", c.lightcyan, f"Datei - Transfer fehlgeschlagen !",
                      c.yellow, 20*" ", "*\r")
            else:
                print(
                      "*", c.lightred, f"Unbekannter Zielrechner   : {z:12s}",
                      c.yellow, 13*" ", "*\r")
                print("*", c.lightred, f"Datei - Transfer fehlgeschlagen !",
                      c.yellow, 20*" ", "*\r")
                dead_ziel = z
            print(60*"*", "\r")
        i += 1
    ...
    try:
        ...
        if not os.path.isfile(input_rsync):
            ...
            if knotenname != "Merkur":
                print(
                      c.lightred, "\r\nError --> Programmausführung auf "
                      "diesem Knoten nicht gestattet !", c.reset, "\n")
            else:
                synchronisation()
